...
# HITRUST/HIPAA and PCI DSS 4.0.1 Policy Initiatives
# =============================================================================
#
...
# - PCI DSS 4.0.1: a06d5deb-24aa-4991-9d58-fa7563154e31
...
# Least-privilege remediation roles are explicit opt-in. Audit assignments work
# without these roles; remediation tasks require the caller to supply the exact
# built-in role definition IDs required by the selected policies.
resource "azurerm_role_assignment" "hipaa_remediation" {
  for_each = var.enable_hipaa ? var.remediation_role_definition_ids : toset([])
...
# -----------------------------------------------------------------------------
# PCI DSS 4.0.1 Policy Initiative
# -----------------------------------------------------------------------------
resource "azurerm_resource_group_policy_assignment" "pci_dss" {
  ...
  display_name         = "PCI DSS 4.0.1 - ${var.environment}"
  description          = "Assigns PCI DSS 4.0.1 policy initiative for payment card industry data security"
  resource_group_id    = var.scope
  ...
  non_compliance_message {
    content = "This resource is not compliant with PCI DSS 4.0.1 requirements."
  }
}

resource "azurerm_role_assignment" "pci_dss_remediation" {
  for_each = var.enable_pci_dss ? var.remediation_role_definition_ids : toset([])
