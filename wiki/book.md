...
- State isolation: each environment (`cheap-lab`, `lab`, `dev`, `prod`) maps to its own key; locks are blob leases.
...
3) 3xx Security - tfsec - SARIF upload (blocking)
4) 3xx Security - Checkov - SARIF upload (blocking)
5) 3xx Security - Secrets - Gitleaks scan
6) 4xx Lint - TFLint - Azure rules (soft-fail)
7) 4xx Lint - Policy - Conftest OPA policies against `tfplan.json` (blocking)
