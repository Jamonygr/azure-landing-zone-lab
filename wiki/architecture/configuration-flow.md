...
- **`environments/*.tfvars`** – tracked, non-secret presets for cheap-lab, lab, dev, and production-like lab profiles.
- **`TF_VAR_*` environment variables** – ephemeral inputs for optional passwords and shared keys.
...
| PaaS | `deploy_functions`, `deploy_container_apps`, `deploy_cosmos_db`, `paas_alternative_location` | Controls optional cloud services. |
